...
The tradeoff is that pseudoalignment does not produce base-level resolution or alignment coordinates, so the output is not suitable for variant calling, allele-specific expression, or splice-junction discovery. It is exactly what is needed for the transcript-level quantification and QC metrics this pipeline emits. End-to-end QC is included as an example for the human genome but mouse samples are accepted by the samplesheet validator and should work as a drop-in.
